const QuestionData = [
  ...
  },
  {
    ...
    answera:
      "'너구리 너무 이쁜 것 같아.. 사람들한테 자랑해야지..!!' 하며 인스타를 만들어서 사진도 공유하고 소통도 한다.",
  ...
  },
  {
    ...
    answera:
      "'우리 고양이 몸무게는 5키로니까 하루세번 몇시에 이만큼 씩 주고, 간식은 이때 챙겨줘야겠다.' 계획을 세운다. ",
...
function clickFunc(point) {
  if (count < 12) {
    ...
    //forEach 같은 반복문 map() ~ map이라는 method는 score를 한바퀴 순회 하면서
    //return을 써줌으로서 새로운 매열을 만들어냄 <무조건 return만 사용>
    ...
    //[reduce] newScore - return 사용 해서 새로운 배열을 반환
    //acc - 누적값, current - 현재 숫자,
  ...
  //많이 쓰는 배열 method map (새로운 배열 리턴), forEach(배열 리턴),find(하나만 찾아서 리턴),filter(배열 리턴)
  count++;
}
